...
function createFloor(x, y, z, color, group) {
    const geometry = new THREE.BoxGeometry(10, 10, 10);
    const material = new THREE.MeshBasicMaterial({ color: color });
    const floor = new THREE.Mesh(geometry, material);
    floor.position.set(x, y, z);
    group.add(floor); // Add floor to the group
...
function createApartment(x, z, group) {
    const floorCount = 4;
    ...
    for (let i = 0; i < floorCount; i++) {
        ...
        createFloor(x, y, z, color, group); // Pass group to createFloor
    }
}

function createApartments(scene, camera) {
    const group = new THREE.Group(); // Create a group
    group.userData = { id: "gptGroup" }; // Set userData
    ...
    for (let i = 0; i < 6; i++) {
        ...
        createApartment(x, z, group); // Pass group to createApartment
    }
...
// Usage example
// Assuming you have already created a THREE.Scene and a THREE.Camera
createApartments(scene, camera);
